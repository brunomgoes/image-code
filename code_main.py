...
def wavelet_clahe(img_array, wave_name): # in: image float64, out img float64
    ...
    #       'Vertical detail', 'Diagonal detail']
    # fig = plt.figure(figsize=(12, 3))
    # for i, a in enumerate([cA_clahe, cH_new, cV_new, cD_new]):
    #     ax = fig.add_subplot(1, 4, i + 1)
    #     ax.imshow(a, interpolation="nearest", cmap=plt.cm.gray)
    #     ax.set_title(titles[i], fontsize=10)
    #     ax.set_xticks([])
...
def segm_3(img_array): # usar o resultado de segm_2 como marcador para watershed
    img = np.copy(img_array)
    im_segm_2 = segm_2(img_array)

    return im_segm_2

def segm_4(img_array): # out: list of dict com fetures de cada ROI
    img = np.copy(img_array)

    roi_list = divide_rois(img)
    data_list = []
    i = 0
    for roi in roi_list:
        roi_array = roi['roi_array']

        k=3
        cA, (cH, cV, cD) = pywt.wavedec2(img, wavelet='coif5', level=1) # decompondo a imagem -> wavelet
        ...
        data_list.append(roi_dict)

    return data_list

def segm_5(img_array):
    img = np.copy(img_array)

    i_th = morphology.white_tophat(img, footprint=np.ones((3,3)))
    
    # wavelet transform of image, and plot approximation and details
    titles = ['Approximation', ' Horizontal detail',
              'Vertical detail', 'Diagonal detail']

    coeffs2 = pywt.wavedec2(i_th, wavelet='coif5', level=1) # decompondo a imagem -> wavelet
    cA, (cH, cV, cD) = coeffs2
